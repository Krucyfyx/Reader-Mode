    ...
    if (Array.isArray(collectedData)) {
      ...
      function paginateTextContent(textArray) {
        ...
        textArray.forEach((textData) => {
          const itemElement = document.createElement('div');
          itemElement.classList.add('injected-text');
          itemElement.textContent = textData.text;
      
          // Apply CSS properties to the itemElement
          const cssProperties = textData.css;
          Object.assign(itemElement.style, cssProperties);
        ...
        totalPages = pages.length;
        showPage(currentPage);
        // contentDiv.style.setProperty("-webkit-column-count", "2");
        contentDiv.style.setProperty("column-count", "2");
      }
